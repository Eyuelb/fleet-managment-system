...
export function formatText(header: string): string {
  const words = header.split(/(?=[A-Z])/);
  return words
    .map((word) => {
      // Ensure the word is not empty
...
export function joinCamelCase(text: string, by: string): string {
  const words = text.split(/(?=[A-Z])/);
  return words.map((word) => word.toLocaleLowerCase()).join(by);
}
